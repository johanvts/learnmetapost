...
The directions are given along the travel direction of the path, so a curve entring a point by going down looks the same as one leaving by going up etc.

The transform =relectedabout= /(p,q)/ can be used to mirror paths about the straight line from pair /p/ to pair /q/.
