...
Since we don't have /functions/ as such, but only macros we can't easily pass fucntions as input into other functions. But there is a way.
...
We can then replace our draw code with a call to the marcro, maybe in a for loop. 
